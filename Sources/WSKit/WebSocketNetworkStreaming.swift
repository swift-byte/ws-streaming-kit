...
///
/// 1. Свой close-код выставляем, только если чужого в задаче ещё нет.
///    `cancel(with:)` записывает код в задачу, а из задачи мы восстанавливаем
///    код СЕРВЕРА, когда didCloseWith не доехал: свой затёр бы чужой. Когда
///    задача пуста, вежливое закрытие бесплатно и полезно — на iOS сервер этот
///    код видит.
...
/// 5. Дедлайн первого сообщения снимается там, где ИСХОД НАЗВАН, — в
///    connectTask перед дренажом, одинаково для всех причин. Дважды он
///    снимался на пути сбоя отправки, и оба ответа на «снимать всегда или
///    только когда закрываем сокет» были неверны: сбой отправки к дедлайну
///    отношения не имеет, а стрим вообще без аплинк-трафика оставался
///    беззащитным. Окно реально: дренаж ждёт читателя, а тот падает не на
///    приходе close-фрейма, а на завершении задачи — целый round-trip.
actor WebSocketNetworkStreaming: NetworkStreaming {
	...
	// поэтому стрим завершает именно отмена задач отсюда. Тихим финиш будет
	// не всегда: если причина уже наблюдена (сервер закрылся, и connectTask
	// стоит в дренаже), она и доедет до потребителя — спросить ушедший актор
	// про вытеснение не у кого, но причина от этого не перестаёт быть причиной
	deinit {
		// Порядок как в cancel(generation:): ввод — до разрыва сокета, иначе
		// inflight-send эскалирует штатное разрушение как сбой аплинка
		inputTask?.cancel()
		Self.closeGracefullyIfUnclaimed(webSocketTask)
		outputTask?.cancel()
		connectTask?.cancel()
		timer?.cancel()
		// finishTasksAndInvalidate, а не invalidateAndCancel: последний рвёт
		// соединение раньше, чем успевает уйти close-фрейм, отправленный строкой
		// выше, и вежливое закрытие снова становится обрывом
	...
	///   либо `cancel()` или деинициализация актора — но только если причина к
	///   тому моменту ещё не известна. Уже наблюдённую причину ни то, ни другое
	///   не гасит: она доезжает до потребителя и после ухода актора;
	...
	/// - Без teardown отказывают ровно два случая, и действующий стрим их
	///   переживает: провал валидации endpoint и отмена вызывающего, замеченная
	///   на входе. Отмена, замеченная ПОЗЖЕ — в окне сбора кук, — приезжает тем
	///   же `CancellationError`, но действующий стрим к тому моменту уже
	///   вытеснен и разрушен. Отличить их по типу ошибки нельзя, поэтому
	///   вызывающий, отменивший свой же вызов, не должен считать прошлый стрим
	///   живым.
	...
	///   исходом не становится — почему именно, см. `noteUplinkFailure`. Сокет
	///   при этом закрывается (кроме случая, когда сервер уже попрощался сам),
	///   поэтому исход, скорее всего, будет тихим финишем без ответа: наша
	///   отмена опережает настоящую причину. Ввод после сбоя не досылается и
	///   терминатор не уходит — оборванная реплика не должна выглядеть целой.
	...
	func establishStream(
		...
		for (key, value) in headers.sorted(by: { $0.key < $1.key }) {
			// Имя и значение отбраковываются по разным правилам, поэтому и в
			// логе разделены: иначе диагностика вела бы не к той половине пары.
			// Имя с управляющими символами Foundation ставит дословно и пустила
			// бы на провод; значение с CR/LF, наоборот, отбрасывает вместе со
			// всем заголовком. Оба исхода одинаково невидимы без лога, а имя
			// логируется очищенным: CR/LF в нём подделали бы запись
			guard Self.isToken(key) else {
				Logger.assistant.error(S("Header rejected, name is not a token: \(Self.logSafe(key))"))
				continue
			}
			guard Self.isValidHeaderValue(value) else {
				Logger.assistant.error(S("Header rejected, value has control characters: \(Self.logSafe(key))"))
				continue
			}
		...
		if Task.isCancelled {
			// ВНИМАНИЕ: предыдущий стрим уже вытеснен и разрушен строками выше,
			// а замены теперь не будет — его потребитель получит
			// CancellationError, который по контракту значит «тебя заменили».
			// Отменять establishStream в окне сбора кук поэтому не стоит:
			// проверка на входе ловит только отмену ДО вытеснения
	...
	/// Свой код закрытия ставим только там, где чужого ещё нет. `cancel(with:)`
	/// ЗАПИСЫВАЕТ код в задачу, а из задачи мы восстанавливаем код сервера,
	...
	/// делегатная очередь может записать чужой код, и наш 1000 его затрёт. Обычно
	/// это ничего не стоит — код приезжает потребителю аргументом `didCloseWith`,
	/// а не из задачи. Но `didCloseWith` теряется (замерено, безотносительно
	/// кода), и вот тогда затёртая задача была единственным носителем: сервер
	/// сказал 1011, а потребитель увидит тихий финиш. Окно оставлено потому, что
	/// закрыть его нечем: задача — не наш стейт, а единственная альтернатива,
	/// прогнать решение через актор, не годится для deinit, где вызов
	/// синхронный. Ширина окна — две инструкции против хопа делегатной очереди,
	/// и совпасть должны обе редкости сразу
	...
	/// Исход назван — снимаем дедлайн первого сообщения. Единственное место,
	/// где он снимается по ходу жизни стрима: там же, где становится известен
	/// исход, и потому одинаково для всех причин — закрытия сервером, обрыва
	/// транспорта, отказа рукопожатия. По поколению, потому что чужой стрим
	/// разоружать не наше дело
	private func disarmForNamedOutcome(generation requested: Int) {
		guard requested == generation else { return }
		disarmTimeout()
	}
	...
	private func createInputTask(
		...
		inputTask = Task.detached { [weak self] in
			...
			/// Сокет закрываем, чтобы не оставить сервер ждать аплинк, а
			/// потребителя — висеть до платформенного дедлайна. Цена известна и
			...
			/// срезала хвост. Признак — код СЕРВЕРА в задаче, и читается он
			/// ПОСЛЕ хопа, в самой точке решения: прочитанный заранее, он к
			/// моменту отмены успел бы устареть ровно в ту сторону, от которой
			/// и защищает. Свой код там оказаться может — teardown вклинивается
			/// между проверкой отмены и чтением, — но решение от этого выходит
			/// лишь консервативнее: закрывать и правда уже нечего.
			/// Дедлайна это не касается — его снимает тот, кто услышал исход
	...
	/// Дедлайн отсюда НЕ снимается, хотя дважды снимался. Сбой отправки — не то
	/// событие: он значит лишь, что транспорта нет, а исход назовёт делегат, и
	/// снимает дедлайн тот, кто исход услышал (`disarmForNamedOutcome`). Пока
	/// снятие висело здесь, вопрос «снимать всегда или только когда закрываем
	/// сокет» не имел верного ответа — оба варианта оставляли окно дренажа
	/// незакрытым для стримов вообще без аплинк-трафика. См. п.5 в шапке
	...
	private func createConnectTask(
		...
		connectTask = Task { [weak self] in
			...
			// Исход назван — причиной или тихим финишем, — и дедлайну больше
			// нечего страховать. А навредить он успевает: поколение двигается
			// только ПОСЛЕ финиша, так что guard в fireTimeout всё это время
			// пропускает, и выстрел подменил бы названную причину на .timeout —
			// «сервер сломался» превратилось бы в «повтори».
			// Тестом не закреплено, и вот почему: окно требует, чтобы дедлайн
			// был ещё взведён, то есть чтобы НЕ пришло ни одного кадра данных
			// (иначе снял бы acceptFirstMessage). Но тогда читателю нечего и
			// дренировать, и окно схлопывается в round-trip close-хендшейка.
			// Условия тянут в разные стороны: попасть в него можно лишь
			// подгадав закрытие к самому дедлайну, и такой тест врал бы в обе
			// стороны. Снятие здесь стоит одного вызова — дешевле, чем
			// рассуждение о том, насколько окно узко на конкретной платформе
			await self?.disarmForNamedOutcome(generation: requested)
			await drainOutput()
	...
	private func fetchReservedCookies() async -> [String: String] {
		...
		// дедлайн) не должно занимать его очередь. Отмена кооперативна, поэтому
		// нечувствительное к ней чтение переживёт дедлайн и отвиснет само;
		// ретраи на заблокированном устройстве такие чтения накапливают —
		// лечится это кэшом на стороне стоража, а не здесь
