...
/// 5. Дедлайн первого сообщения после сбоя отправки снимается всегда, а сокет
///    закрывается лишь при отсутствии чужого кода. Решения разные, и связывать
///    их не надо: транспорта после сбоя нет в любом случае, исход назовёт
///    делегат, а оставленный дедлайн успеет выстрелить в окне дренажа и
///    подменить чужой 1011 на `.timeout`.
actor WebSocketNetworkStreaming: NetworkStreaming {
	...
	private func createInputTask(
		...
		inputTask = Task.detached { [weak self] in
			...
			/// Дедлайн при этом снимается в любом случае, см. `noteUplinkFailure`
	...
	/// Дедлайн первого сообщения снимается ВСЕГДА — независимо от того, закрываем
	/// ли мы сокет вслед за этим. Связывать эти два решения не надо: сбой
	/// отправки значит, что транспорта уже нет, и исход в обоих случаях назовёт
	/// делегат — при чужом коде по состоявшемуся закрытию, при своём по нашей же
	/// отмене. Страховать дедлайну нечего, а навредить он успевает: выстрелив в
	/// окне дренажа, он подменит настоящую причину на `.timeout`, и потребитель
	/// примет серверный 1011 за «повтори». Пробовали иначе — см. п.5 в шапке
